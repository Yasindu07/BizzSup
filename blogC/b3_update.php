...
if(isset($_POST['update_blog'])) {
    ...
    if(empty($blog_id) || empty($blog_title) || empty($blog_content) || empty($blog_date) || empty($blog_image) ){
        ...
        $update_data = "UPDATE blog3 SET bid ='$blog_id', title ='$blog_title', content ='$blog_content', bdate ='$blog_date', image ='$blog_image' WHERE id ='$id'";
...
<?php 
$select = mysqli_query($conn,"SELECT * FROM blog3 WHERE id = '$id' ");
while($row = mysqli_fetch_assoc($select)){


?>      <!-- display update panel for enter date -->
        <form action ="" method ="post" enctype ="multipart/form-data">
        <h3> Update the Blog 03 </h3>
        <input type="text"  class ="box" name ="blog_id" value="<?php echo $row['bid'];?>" placeholder ="Enter blog ID like (D_)">
        ...
        <a href="blog3_panel.php" class="btn" >go back </a>
</form>
